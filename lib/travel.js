...
HOTELL — VAR KONKRET
- Ange alltid hotellets riktiga namn, stadsdel och betyg (0–10, Booking-skala).
- Hittar du inget betyg: sök efter det. Går det ändå inte, sätt rating till null
  och skriv varför i uncertainties. Gissa aldrig ett betyg.
- Skriv i hotel.note vad som utmärker läget: gångavstånd till vad, hur långt
  till nattlivet/centrum. Det är läget som avgör om resan blir bra.
...
async function findTrips(rawCriteria, onProgress = () => {}) {
  ...
  return {
    ...
    trips: trips.map((t, i) => shapeTrip({ ...t.base, originCity: c.origin }, t.pick, i + 1)),
    diagnostics,
  };
}
...
function shapeTrip(base, pick, rank) {
  ...
  return {
    ...
    booking: booking.linksFor({
      origin: base.originCity,
      destination: base.destination,
      originCode: base.outbound?.fromAirport,
      destinationCode: base.airportCode || base.outbound?.toAirport,
      outDate: base.outbound?.date,
      inDate: base.inbound?.date,
      hotelName: base.hotel?.name,
      nights: base.nights,
    }),
