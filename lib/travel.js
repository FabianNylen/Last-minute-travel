...
function enforceHardConstraints(candidates, c) {
  ...
  for (const raw of candidates) {
    ...
    const outDep = combine(cand.outbound?.date, cand.outbound?.departTime);
    const outArr = combine(cand.outbound?.date, cand.outbound?.arriveTime);
    const inDep = combine(cand.inbound?.date, cand.inbound?.departTime);
    const inArr = combine(cand.inbound?.date, cand.inbound?.arriveTime);
...
async function findTrips(rawCriteria, onProgress = () => {}) {
  ...
  await Promise.all(kept.map(async (cand) => {
    const from = combine(cand.outbound?.date, cand.outbound?.arriveTime) || c.earliestDeparture;
    const to = combine(cand.inbound?.date, cand.inbound?.departTime) || c.latestReturn;
...
function shapeTrip(base, pick, rank) {
  const outDep = combine(base.outbound?.date, base.outbound?.departTime);
  const outArr = combine(base.outbound?.date, base.outbound?.arriveTime);
  const inDep = combine(base.inbound?.date, base.inbound?.departTime);
  const inArr = combine(base.inbound?.date, base.inbound?.arriveTime);
...
module.exports = {
  ...
  ValidationError,
  STYLE_LABELS,
  _internals: { parseLocal, combine, formatSwedishDateTime, num },
};
